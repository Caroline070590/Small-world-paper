...
├── notebooks_py/                   # verbatim .py conversion of EVERY notebook
└── notebooks/                      # original Jupyter notebooks (outputs stripped)
```

Every notebook is also provided as a faithful `.py` script in `notebooks_py/`
(code reproduced verbatim, markdown kept as comments, all files compile and were
line-checked against their source). See `notebooks_py/README.md` for the
...
## ⚠️ Important: two representation flavours

GAF and MTF are implemented **two different ways** in this project, and they are
**not interchangeable** — they produce different matrices and therefore different
metrics. Both are preserved on purpose:

| Flavour     | GAF / MTF computed on…                | Used by                          |
|-------------|----------------------------------------|----------------------------------|
| `fmri`      | the **full signal**, then cropped to `Q×Q` | BASC-122 fMRI pipeline           |
| `lengthQ`   | the signal **resampled to length `Q`** first | ECG, EMG, respiration, sleep, MEG |

In code these are `calculate_gaf` / `calculate_mtf` versus
`calculate_gaf_from_lengthQ` / `calculate_mtf_from_lengthQ`. Do **not** unify them
without re-running the affected pipelines.

A second, related distinction: the **synthetic-figure** robustness code
(`analysis/synthetic_generators_and_robustness.py`) uses **mean clustering** in
the random null model, whereas the **biological** pipeline
(`src/smallworld_qtn/network_metrics.py`) uses **transitivity** (matching Eq. 10).
...
* **Simulated MEA** is *generated by code*, not stored. Raw recordings are
  produced with the external SpikeSense / MEArec generator (linked above);
