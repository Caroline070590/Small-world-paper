"""Synthetic time-series generators (i.i.d., pink, AR(1), sinusoid, fBm-like,
logistic map, regime-switching) and the sigma-vs-Q robustness analysis
(Fig. 2, Fig. 3). NOTE: this synthetic pipeline uses MEAN clustering for the
null-model comparison, not transitivity; it is intentionally kept separate
from smallworld_qtn.network_metrics (which uses transitivity for the
biological datasets).

Provenance: extracted verbatim from the notebook ``all-funtions.ipynb``.
