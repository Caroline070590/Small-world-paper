#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Auto-converted from the Jupyter notebook ``ds000117-final.ipynb``.
...
# [magic] !$CONDA env remove -n $ENV_NAME -y

# create using ONLY conda-forge
# [magic] !$CONDA create -y --override-channels -c conda-forge -n $ENV_NAME \
# [magic-cont]     python=3.11 datalad git-annex git mne numpy pandas scipy networkx tqdm ipykernel p7zip

# register the kernel
# [magic] !$CONDA run -n $ENV_NAME python -m ipykernel install --user --name $ENV_NAME --display-name "Python (datalad_meg)"

# In[9]:

# [magic] !$CONDA run -n $ENV_NAME datalad --version
# [magic] !$CONDA run -n $ENV_NAME git annex version
