#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Auto-converted from the Jupyter notebook ``Boxplot-ecg.ipynb``.
...
# ---------- input files ----------
FILES = {
    "QTN": "out_nsrdb/metrics_QTN_nsrdb.csv",
    "GAF": "out_nsrdb/metrics_GAF_nsrdb.csv",
    "MTF": "out_nsrdb/metrics_MTF_nsrdb.csv",
...
OUT_DIR         = "Boxplot-ecg"
OUT_PLOT_PNG    = "sigma_violin_box_points_lines-nsrdb.png"
OUT_PLOT_PDF    = "sigma_violin_box_points_lines-nsrdb.pdf"
OUT_WIDE_CSV    = "sigma_wide_matched-nsrdb.csv"
OUT_LONG_CSV    = "sigma_long_matched-nsrdb.csv"
...
# ---------- input files ----------
FILES = {
   "QTN": "out_fantasia/metrics_QTN_fantasia_all.csv",
    "GAF": "out_fantasia/metrics_GAF_fantasia_all.csv",
    "MTF": "out_fantasia/metrics_MTF_fantasia_all.csv"
}
...
def _find_sigma_col(df: pd.DataFrame) -> Optional[str]:
    norm_map = {_norm(c): c for c in df.columns}
    for a in SIGMA_ALIASES:
        if _norm(a) in norm_map:
            return norm_map[_norm(a)]
...
def _load_sigma(path: str, dataset: str) -> pd.DataFrame:
    ...
    c_sigma = _find_sigma_col(df)
    if c_sigma is None:
        print(f"[WARN] {dataset}: no sigma column in {path}. Columns: {list(df.columns)}")
        return pd.DataFrame()

    out = pd.DataFrame({
        "patient_id": ids,
        "key": ids,   # IMPORTANT: keep original IDs exactly
        dataset: pd.to_numeric(df[c_sigma], errors="coerce")
    })
...
def main():
    ...
    print(f"Matched subjects: {len(wide)}")
    print(wide.head())
