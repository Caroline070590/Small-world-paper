#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Auto-converted from the Jupyter notebook ``Boxplot-final.ipynb``.
...
METRICS_TO_PLOT = {
    ...
    "omega": {
        "ylabel": r"Small-worldness $\omega$",
        "threshold": 0.0,
        "threshold_label": r"$\omega$ = 0.0",
        "group": "small_world"
    },
    "phi": {
        "ylabel": r"Small-worldness $\phi$",
        "threshold": None,
...
def normalize_key(s: str) -> str:
    s = str(s).strip()
    # fantasia often works better if exact patient_id is preserved
    return s
...
def load_metric_table(path: str, rep: str, metric: str) -> pd.DataFrame:
    ...
    if "patient_id" in df.columns:
        ...
        ids = pd.Series([f"{rep}_{i}" for i in range(len(df))], name="patient_id")
...
def merge_three_reps(qtn: pd.DataFrame, gaf: pd.DataFrame, mtf: pd.DataFrame) -> pd.DataFrame:
...
def normalize_key(s: str) -> str:
    # keep exact IDs; this is safer for fantasia and avoids over-normalization
    return str(s).strip()
