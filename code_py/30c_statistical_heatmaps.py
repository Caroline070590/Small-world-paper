#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Auto-converted from the Jupyter notebook ``Statitical-test-final-heatmaps.ipynb``.
...
# --------------------------------------------------
# CLEAN TRIANGULAR CLUSTERED SIMILARITY HEATMAP
# --------------------------------------------------
def plot_triangular_clustered_similarity(sim_df: pd.DataFrame, out_dir: str, summary_stat="median"):
    ...
    ax_heat.set_xticklabels(labels, rotation=35, ha="right", fontsize=10)
    ax_heat.set_yticklabels(labels, rotation=0, fontsize=11)
    ax_heat.set_xlabel("Dataset", fontsize=13)
    ax_heat.set_ylabel("Dataset", fontsize=13)

    # colorbar in white space
    cax = fig.add_axes([0.08, 0.80, 0.18, 0.03])
    ...
    cax.set_title("Similarity", fontsize=11)
...
# --------------------------------------------------
# OPTIONAL: clustered metric profile heatmap
# --------------------------------------------------
def plot_clustered_metric_profiles(profile_df: pd.DataFrame, out_dir: str, summary_stat="median"):
    ...
    ax_heat.set_xlabel("Complex network metrics", fontsize=13, labelpad=10)
    ax_heat.set_ylabel("Dataset", fontsize=13, labelpad=10)
    ax_heat.set_xticklabels(ax_heat.get_xticklabels(), rotation=30, ha="right", fontsize=10)
    ...
    cax.set_title("Z-score", fontsize=11)
    cax.tick_params(labelsize=9)
